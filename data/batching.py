    ...
    def __iter__(self):
        ...
        for i in range(self.total_n_classes):
            if i == 0:
                ...
                cls_idxs.append(list(range(self.dataset.cls_end_idx[i - 1], self.dataset.cls_end_idx[i])))
...
def pad_batch(batch):
    ...
    for key, values in packed_dict.items():
        if key in ["cls", "num_frames", "num_steps"]:
            batch_dict[key] = torch.tensor([int(v) for v in values])
            # batch_dict[key] = torch.stack(values, 0)
        if key in ["frame_features", "step_features"]:
            max_len = max([t.shape[0] for t in values])
            padded_batch = [F.pad(t, (0, 0, 0, max_len - t.shape[0]), value=0) for t in values]
            batch_dict[key] = torch.stack(padded_batch, dim=0)
        if key in ["step_ids", "step_starts", "step_ends", "step_starts_sec", "step_ends_sec"]:
            max_len = max([t.shape[0] for t in values])
            padded_batch = [F.pad(t, (0, max_len - t.shape[0]), value=-1) for t in values]
            batch_dict[key] = torch.stack(padded_batch, dim=0)
        if key in ["name", "cls_name"]:
            # packed_dict[key] = '||'.join(values)
            pass
    ...
    frame_pad_mask = torch.zeros(batch_dict["frame_features"].shape[:2])
    for i, num_frames in enumerate(batch_dict["num_frames"]):
        frame_pad_mask[i, num_frames:] = 1
    batch_dict["frame_pad_mask"] = frame_pad_mask.to(bool)
    batch_dict["step_pad_mask"] = batch_dict["step_ids"] == -1
